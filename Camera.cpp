...
namespace gps {
	...
	void Camera::move(MOVE_DIRECTION direction, float speed) {
		...
		if (direction == MOVE_FORWARD)
			cameraPosition += cameraFrontDirection * (speed + 0.1f);
		if (direction == MOVE_BACKWARD)
			cameraPosition -= cameraFrontDirection * (speed + 0.1f);
		if (direction == MOVE_LEFT)
			cameraPosition -= cameraRightDirection * (speed + 0.1f);
...
;
	...
	void Camera::setCameraPosition(glm::vec3 a)
	{
		cameraPosition = a;
	}

	void Camera::setCameraTarget(glm::vec3 a)
	{
		cameraTarget = a;
	}
